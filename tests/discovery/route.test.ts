...
describe('discovery API', () => {
  ...
  it('returns a compact ready preview with no-store headers', async () => {
    vi.mocked(previewGames).mockResolvedValue({ status: 'ready_for_approval', previewId: 'id', expiresAt: 'date',
      query: 'horror', validation: {} as never, candidates: [{ steamAppId: 1, igdbId: 2, name: 'Game', reason: 'Match', matchedTags: ['Horror'] }],
    ...
    expect(body.data.candidates[0]).toEqual({ steamAppId: 1, igdbId: 2, name: 'Game', reason: 'Match', matchedTags: ['Horror'] });
