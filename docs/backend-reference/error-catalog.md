...
| `INVALID_AI_OUTPUT` | 503 | Ranking membership or tag checks failed |
...
A successful preview can still report `no_matches` or `complete:false`; these are valid discovery outcomes, not transport errors. A successful approved collection can contain per-game failures alongside usable games.
